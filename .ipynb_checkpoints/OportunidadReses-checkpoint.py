...
Tasa_merma= col0.slider("% de merma", 0., 0.2,0.065)

st.write('                    ')

# st.subheader('Resultados')

resultado = calcular_beneficio_integracion(Precio_KG_Res_Gorda_n12, Precio_KG_Res_Flaca_n,Peso_Inicial_Res_n,Por_Beneficio_Integrado,Peso_Final_Res_n12,Gasto_Operativo_Anual,Cantidad_Reses,Gasto_Transporte,Tasa_Costo_Capital,Cantidad_Reses_Simular,Tasa_merma,"metricas")
EVA=resultado[6]
...
calculos = ["Valor Res Flaca (Inversión)","Valor Res Gorda","Utilidad Neta","UODI",f"Costo de Capital: {'{:,.0f}%'.format(Tasa_Costo_Capital*100)}","EVA","ROIC" ]
...
with st.expander('Escenario Valorizado 2'):
    ...
    Tasa_merma_e2= col0.metric("% de merma", "{:,.0000f}%".format(round(Tasa_merma*100)))
    ...
    resultado2 = calcular_beneficio_integracion(Precio_KG_Res_Gorda_n12_e2, Precio_KG_Res_Flaca_n_e2,Peso_Inicial_Res_n_e2,Por_Beneficio_Integrado_e2,Peso_Final_Res_n12_e2,Gasto_Operativo_Anual_e2,Cantidad_Reses_e2,Gasto_Transporte_e2,Tasa_Costo_Capital,Cantidad_Reses_Simular_e2,Tasa_merma,"metricas")
    EVA_e2=resultado2[6]
...
#     key='download-csv2')
    ...
    if EVA > EVA_e2:
        ...
        Eva1Eva2 = abs((abs(EVA_e2)-abs(EVA)))
